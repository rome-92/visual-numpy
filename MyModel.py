...
class MyModel(QAbstractTableModel):
    def __init__(self, parent=None):
        ...
        self.history = []
        self.history.append((
            self.dataContainer.copy(), copy.deepcopy(self.formulas),
            self.alignmentDict.copy(), self.fonts.copy(),
            self.foreground.copy(), self.background.copy()
            ))
    ...
    def mimeData(self, indexes, flag='mouse'):
        ...
        if flag == 'mouse':
            ...
            currentIndexColumn = str(
                self.parent().currentIndex().column()
                )
            ...
            for row in range(beginRow, endRow, stepRow):
                for column in range(beginColumn, endColumn, stepColumn):
                        ...
                        if f := self.formulas.get((row, column), None):
                            try:
                                self.checkForCircularRef(
                                    f, newRowDifference, newColumnDifference
                                    )
                                f.addressRow = f.addressRow + newRowDifference
                                f.addressColumn = \
                                    f.addressColumn + newColumnDifference
                                self.formulas[(
                                    row + newRowDifference,
                                    column+newColumnDifference)] = f
                    ...
                    selectionModel.select(
                        movedIndex, QItemSelectionModel.Select
                        )
            self.updateModel_()
    ...
    def checkForCircularRef(self, formula, *deltas):
        """Check for circular reference if formula is to be dropped"""
        rowsDomain = []
        columnsDomain = []
    ...
    def updateModel_(self):
        """Execute formulas if any are pending of updating"""
        for f in self.ftoapply:
            if f in self.formulas.values():
                self.parent().parent().calculate(
                    f.text, f.addressRow, f.addressColumn, flag=True)
        self.ftoapply.clear()
    ...
    def insertColumns(self, column, count, parent=QModelIndex()):
        """Insert 'count' number of columns"""
    ...
    def getAlphanumeric(self, column, row):
        """Get the alphanumeric coordinate for the corresponding cell"""
        if column < 26:
    ...
    def headerData(self, section, orientation, role=Qt.DisplayRole):
        """Return the letter or letters for the corresponding column"""
...
class CircularReferenceError(Exception):
    ...
    def __str__(self):
        errorInfo = \
            "Formula in row {}, column {}, creates circular reference".format(
                self.formulaRow,
                self.formulaColumn
                )
        return errorInfo
