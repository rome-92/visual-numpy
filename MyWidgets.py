...
from PySide6.QtWidgets import (
    QMainWindow, QLineEdit, QToolBar, QLabel, QFileDialog,
    QMessageBox, QFontComboBox, QComboBox, QColorDialog,
    QPushButton
    )
from PySide6.QtGui import (
    QAction, QGuiApplication, QActionGroup, QFont,
...
class MainWindow(QMainWindow):
    ...
    def createNew(self):
        """Create a new file and clears history"""
    ...
    def importFile(self):
        ...
        name, notUsed = QFileDialog.getOpenFileName(
            self, 'Import File', '', 'csv files (*.csv)'
            )
        if name:
            try:
                with open(name, encoding='latin', newline='') as myFile:
                    ...
                    for rowNumber, row in enumerate(reader):
                        for columnNumber, column in enumerate(row):
                            index = self.view.model().createIndex(
    ...
    def pasteAction(self):
        ...
        self.view.model().dropMimeData(
            self.mimeDataToPaste, self.pasteMode, -1, -1, parent
            )
    ...
    def mergeCells(self):
        ...
        if height * width == len(selectedIndexes):
            self.view.setSpan(
                topLeftIndex.row(), topLeftIndex.column(), height, width
                )
    ...
    def unmergeCells(self):
        ...
        if len(selectedIndexes) > 1:
            ...
            if height * width == len(selectedIndexes):
                self.view.setSpan(
                    topLeftIndex.row(), topLeftIndex.column(), 1, 1
                    )

    def saveArrayAs(self):
        """Save array into .npy array format"""
        name, notUsed = QFileDialog.getSaveFileName(
            self, 'Save Array', '', 'npy files(*.npy)'
            )
        if name:
            ...
            if height * width == len(selected):
                ...
                for y, row in enumerate(range(topRow, bottomRow + 1)):
                    for x, column in enumerate(range(
    ...
    def updateFont(self):
        ...
        for index in selected:
            self.view.model().setData(
                index, globals_.currentFont, role=Qt.FontRole
                )
        self.view.model().dataChanged.emit(selected[0], selected[-1])
        self.view.saveToHistory()
    ...
    def italic(self):
        ...
        for index in selected:
            font = QFont(model.fonts.get(
                (index.row(), index.column()), globals_.defaultFont
                ))
            if self.sender().isChecked():
                font.setItalic(True)
            else:
    ...
    def showColorDialog(self):
        ...
        """ % (
            fontR, fontG, fontB, fontA, backgroundR,
    ...
    def calculate(self, text, *resultIndex, flag=False):
        """Formats corresponding string into python
        executable code and evaluates resulting expression
        """
        ...
        for array in arrays:
            ...
            for y, row in enumerate(range(rows[0], rows[1])):
                for x, column in enumerate(range(columns[0], columns[1])):
                    element = \
                        self.view.model().dataContainer.get((row, column), 0)
                    try:
        ...
        if type(result) is np.ndarray:
            ...
            self.view.model().formulaSnap = list(
                self.view.model().formulas.values()
                )
            if len(result.shape) == 2:
                ...
                for line, rY in zip(
                        result, range(
                            resultIndexRow, resultIndexRow+resultRows)):
                    for dE, cX in zip(
                            line,
                            range(
                                resultIndexColumn,
                                resultIndexColumn+resultColumns)):
                        ind = self.view.model().createIndex(rY, cX)
                        self.view.model().setData(
                            ind, globals_.currentFont, role=Qt.FontRole
                            )
                        self.view.model().setData(
                            ind, dE, formulaTriggered=True
                            )
                startIndex = self.view.model().index(
                    resultIndexRow, resultIndexColumn
                    )
                ...
                endIndex = self.view.model().index(
                    resultIndexRow+resultRows-1, resultIndexColumn
                    )
            else:
                return
            ...
            self.view.model().setData(
                startIndex, globals_.currentFont, role=Qt.FontRole
                )
            self.view.model().setData(
                startIndex, result, formulaTriggered=True
                )
            self.view.model().dataChanged.emit(startIndex, endIndex)
            self.commandLineEdit.clearFocus()
        if not flag:
